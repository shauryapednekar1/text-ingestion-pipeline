...
class TestLoader(unittest.TestCase):
    ...
    def test_unzip_dataset(self, mock_unzip, mock_makedirs, mock_path):
        ...
        Ensures that the necessary directories are created and that the dataset
        is correctly unzipped into the specified directory.
        """
        mock_path.join.return_value = "unzipped/dataset"
        mock_path.dirname.return_value = "dirname"
        mock_path.basename.return_value = "dataset.zip"
        directory = self.loader.unzip_dataset(
            "input_dir/dataset.zip", "unzipped"
        )
